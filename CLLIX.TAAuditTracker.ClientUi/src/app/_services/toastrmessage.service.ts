...
export class ToastrmessageService {
  ...
  showValidationWarnings(errors?: Record<string, string[]>, customTitle = 'Validation Warning') {
    if (!errors) return;
  ...
  showDetailedError(response: CustomResultResponse, customTitle = 'Submission Error') {
    let errorToast = response.message ? ` ${response.message}` : ' An error occurred.';
